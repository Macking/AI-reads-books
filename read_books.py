...
ANALYSIS_INTERVAL = 10  # Set to None to skip interval analyses, or a number (e.g., 10) to generate analysis every N pages
MODEL = "ep-xxxxxxxxxxxxxxxx-xxxxx"
ANALYSIS_MODEL = "ep-xxxxxxxxxxxxxxxx-xxxxx"
...
def process_page(client: OpenAI, page_text: str, current_knowledge: list[str], page_num: int) -> list[str]:
    ...
    response_completion =  client.chat.completions.create(
        ...
        messages=[
            ...
            - One knowledge point limit to 1-2 sentenses with no more than 50 words
            ...
            For answer:
            - Retrict for json formt
            - Donnot appear illegal character in json string"""},
            {"role": "user", "content": f"Page text: {page_text}"}
        ]
    )
    
    #completion = PageContent(**response_completion)
    #result = completion.choices[0].message.parsed
    result = response_completion.choices[0].message.content
    result_json = json.loads(result)
    knowledge = []
    if 'knowledge' in result_json:
        knowledge = result_json['knowledge']
    if 'knowledge_points' in result_json:
        knowledge = result_json['knowledge_points']
    if result_json['has_content']:
        print(colored(f"✅ Found {len(knowledge)} new knowledge points", "green"))
    else:
        print(colored("⏭️  Skipping page (no relevant content)", "yellow"))

    parsed_knowledge = []
    for index in range(len(knowledge)):
        content = knowledge[index]
        if 'point' in content:
            parsed_knowledge.append(content['point'])
        else:
            parsed_knowledge.append(content)

    updated_knowledge = current_knowledge + parsed_knowledge
...
def main():
    ...
    setup_directories()
    client = OpenAI(base_url="https://ark.cn-beijing.volces.com/api/v3/", api_key=os.environ.get("ARK_API_KEY"))
